...
var cn1 = {
    ...
    stakingtracker_addr:"0xe29dA308B6e8229Ba0134c71490451ad6ACfcc7e",
}
...
var cn2 = {
    ...
    stakingtracker_addr:"0x7C3e7564Cfa7063Bc10Fd921d82a64c589A6EE84",
}
...
async function deploy_stakingtracker(cn) {
  var c = new caver.klay.Contract(stakingtracker.abi)

  console.log(`[${cn.id}] Deploying StakingTracker contract...`)
  var p = c.deploy({
  ...
  var r = await p;
  cn.stakingtracker_addr = r._address;
  console.log(`[${cn.id}] staking tracker contract addr = ${cn.stakingtracker_addr}`);
}
...
async function init_v2(cn) {
  ...
  await c.methods.setStakingTracker(cn.stakingtracker_addr).send({from:cn.admin.address, gas:4000000, value:0})
...
async function setupCnV2() {
  ...
  await deploy_stakingtracker(cn1);
  await deploy_stakingtracker(cn2);
