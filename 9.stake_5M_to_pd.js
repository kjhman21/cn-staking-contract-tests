...
async function stakeKLAY(amountInKlay) {
  var pdc = new caver.klay.Contract(pd.abi, cn1.pd_addr)

  console.log(`Staking ${amountInKlay} KLAY to pd`);
  await pdc.methods.stake().send({from:cn1.new_admin.address, gas:4000000, value:caver.utils.toPeb(amountInKlay, "KLAY")})
}

async function checkStaking() {
  // stake to pd
  await stakeKLAY(5_000_000);
