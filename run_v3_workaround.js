...
var pd_addr = "0xFc756C6D2F2e48711C57027F4532b79CECf78685";
...
async function deploy_v3() {
  ...
  var p = c.deploy({
    data: cnv3.bytecode,
    arguments: [u1.address, u2.address, u1.address, [addr2], 1, [], []]
  })
...
async function deploy_pd() {
  ...
  var p = c.deploy({
    data: pd.bytecode,
    arguments:[cnv3_addr, [u1.address, u1.address, 1000, 'GC1']]
  })
  .send({ from: u1.address, gas: 40000000, value: 0 })

  var r = await p;
  pd_addr = r._address;
  console.log("pd contract addr = " +pd_addr);
}
...
async function init_v3() {
  ...
  // link cnStakingV3 and public delegation
  await deploy_pd()
}
...
async function updateCnStakingReward() {
  var c = new caver.klay.Contract(cnv3.abi, cnv3_addr);
  var pdaddr = await getPdAddr()
  var pdc = new caver.klay.Contract(pd.abi, pdaddr)

  console.log('UpdateRewardAddress..')
  console.log(await c.methods.submitUpdateRewardAddress(pd_addr).send({from:u2.address, gas:400000, value:0}))

  // console.log("confirmRequest")
  // var ids = await c.methods.getRequestIds(0,10000,requestStates.NotConfirmed).call({from:u2.address, gas:4000000, value:0})

  // var firstArg = caver.utils.padLeft(pd_addr,64);
  // var secondArg = caver.utils.padLeft('0x0', 64);
  // var thirdArg = caver.utils.padLeft('0x0', 64);

  // console.log(await c.methods.confirmRequest(ids[0], functionIds.UpdateRewardAddress, firstArg, secondArg, thirdArg).send({from:u2.address, gas:4000000, value:0}));

  console.log('acceptRewardAddress..')
  console.log(await pdc.methods.acceptRewardAddress().send({from:u1.address, gas:400000, value:0}))
}
...
async function constructAddressBook() {
  ...
  console.log(await c.methods.submitRegisterCnStakingContract(u2.address, cnv3_addr, u1.address).send({from:u1.address, gas:4000000}))

  // update reward address
  await updateCnStakingReward();
...
async function addPdToAdmin() {
  var c = new caver.klay.Contract(cnv3.abi, cnv3_addr);
  var pdaddr = await getPdAddr();

  // add pd in the admin list
  console.log("adding pd in the admin list...")
  console.log(await c.methods.submitAddAdmin(pdaddr).send({from:u2.address, gas:400000, value:0}))
}
...
//   let ADMIN_ROLE = await c.methods.ADMIN_ROLE().call({from:u1.address, gas:4000000, value:0})
//   var admins = await c.methods.getRoleMemberCount(ADMIN_ROLE).call({from:u1.address, gas:400000, value:0})
//   console.log(`ADMIN_ROLE = ${ADMIN_ROLE}`)
//   for(var i = 0;i < admins; i++) {
//     console.log(await c.methods.getRoleMember(ADMIN_ROLE, i).call({from:u1.address, gas:400000, value:0}))
//   }
// }
